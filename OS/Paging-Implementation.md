...
재귀 페이징은 페이지 테이블의 매핑 하나가 얼마나 강력한지 보여준다. 구현과 설정이 쉬우므로(재귀 항목 한 개만 추가하면 끝) 페이징 구현에 사용하기 적합하다.

하지만 단점도 있다.

- 대량의 가상 메모리(512GB)를 차지한다. 큰 48비트 주소 공간에서 문제는 되지 않지만 캐싱하기가 어려울 수 있다.
- 현재 활성화된 주소 공간에만 접근하기 쉽다. 다른 주소 공간에 접근하기 위해서는 재귀 항목을 변경해야 한다. 
- x86의 페이지 테이블 형식에 의존하므로 다른 아키텍처에서 동작하지 않을 수 있다.
